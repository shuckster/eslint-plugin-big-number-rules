...
const { makeSettingGetter, getConstruct, getSumMethod } = require('../settings')
const { withImportDeclaration, cleanArray } = require('../helpers')
const { isWorkable } = require('./utils/infix-inspection')
const { isBigNumberChain } = require('./utils/detect-chain.js')
const { makeOperatorGuard } = require('./utils/operator-guard.js')
...
const allArithmeticOperators = Object.keys(arithmeticMethods)
const [abiguousOps, unambiguousOps] = sift(allArithmeticOperators, '+')
...
function arithmeticEntry(context) {
  ...
  const isAmbiguousOp = makeOperatorGuard(abiguousOps)(context)
  const isUnambiguousOp = makeOperatorGuard(unambiguousOps)(context)
  const SuggestArithmeticReplacement = SuggestReplacement

  return against(
    when({
      type: 'BinaryExpression',
      operator: isUnambiguousOp
    })(SuggestArithmeticReplacement),

    when({
      type: 'BinaryExpression',
      operator: isAmbiguousOp,
      left: isWorkable(context),
      right: isWorkable(context)
    })(SuggestArithmeticReplacement),
  ...
  function SuggestReplacement(node) {
    ...
    const { operator } = node
    const L = context.sourceCode.getText(node.left)
    const R = context.sourceCode.getText(node.right)
