...
function arithmeticEntry(context) {
  ...
  function getArithmeticMethod(L, operator, R) {
    ...
    const [negate, ...rest] = result
    const negation = negate === '__NEGATION__'
    return [negation, ...rest]
  }

  function bigNumberArithmetic(L, operator, R) {
    const construct = getConstruct(context)
    const [negation, left, method, right] = getArithmeticMethod(L, operator, R)
    const negate = negation ? '!' : ''
    return `${negate}${construct}(${left}).${method}(${right})`
  }

  function bigNumberAddToChain(L, operator, R) {
    const [negation, , method, right] = getArithmeticMethod(L, operator, R)
    const negate = negation ? '!' : ''
    return `${negate}${L}.${method}(${right})`
  }
    ...
    stack.push({
      ...
      report: {
        ...
        fix: fixer => {
          ...
          return fixer.replaceText(node, bigNumberArithmetic(L, operator, R))
        }
      }
    })
  }
