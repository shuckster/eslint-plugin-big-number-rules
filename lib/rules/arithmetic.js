...
function arithmeticEntry(context) {
  ...
  const supportedOperators = Object.keys(arithmeticMethods)

  function getArithmeticMethod(L, operator, R) {
    const method = arithmeticMethods[operator]
    const K = (L, R) => [L, method, R]
    const fn = typeof method === 'function' ? method : K
    return fn(L, R)
  }
  ...
  return node => {
    if (!supportedOperators.includes(node.operator)) {
      return
    }

    const original = context.getSource(node)
    const summable = isSummable(node)
...
module.exports = {
  create: context => ({
    BinaryExpression: arithmeticEntry(context),
    'BinaryExpression:exit': arithmeticExit(context)
  }),
  meta: {
